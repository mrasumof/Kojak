...
counter = 1
total = coll_articles.find({"source":"LaNacion"}).count()

for each in coll_links.find({"source":"LaNacion"}):

    #print "Gathering:"+each['link']
    article_link = each['link']

    #Check if the article exists in the DB
    exist = coll_articles.find_one({"link": article_link})
    ...
    if exist == None:
        ...
        article_object = {"link":article_link, "article":page, "source:":"LaNacion"}
